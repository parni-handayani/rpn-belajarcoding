...
*/
  ...
  function shoppingTime(memberId, money) {
    let shop = {}, sisaUang, listPurchased = [];
    let listItems = [ "Sepatu Stacattu", "Baju Zoro", "Baju H&N", "Sweater Uniklooh", "Casing Handphone" ];
    let harga = [1500000, 500000, 250000, 175000, 50000];
    
    if (memberId === undefined || memberId === ""){
      return "Mohon maaf, toko X hanya berlaku untuk member saja";
    } else if ( money < harga[harga.length-1]){
      ...
      for (let i = 0; i < harga.length; i++){
        if (money >= harga[i]){
          money -= harga[i];
          sisaUang = money;
          listPurchased.push(listItems[i]);
          
        } 
      }
      shop.listPurchased = listPurchased;
      shop.changeMoney = sisaUang;
      return shop; 
    }
  }
