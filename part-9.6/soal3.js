...
function countProfit(shoppers) {
    ...
    for (let i = 0; i < listBarang.length; i++){
      ...
      let arrShoppers = [], profit = 0;
      for (let j = 0; j < shoppers.length; j++){
        
        let nameShoppers = shoppers[j].name;
        let productShoppers = shoppers[j].product;
        let amountProduct = shoppers[j].amount;
        
        if (productShoppers === produk && stock >= amountProduct){
          arrShoppers.push(nameShoppers);
          stock -= amountProduct;
          profit += amountProduct * harga;
        }
      }
